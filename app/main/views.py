...
def index():
    ...
    if show_followed:
        ...
        query = Post.query
    pageination = Post.query.order_by(Post.timestamp.desc()).paginate(
...
def post(id):
    ...
    paginaton = post.comments.order_by(Comments.timestamp.asc()).paginate(
...
def edit(id):
    ...
    if current_user != post.author and \
            not currnet_usre.can(Permission.ADMINISTER):
        abort(403)
    form = PostForm()
    if form.validate_on_submit():
...
def moderate():
    ...
    pagination = Comment.query.order_by(Comment.timestamp.desc()).paginate(
        page, per_page=current_app.config['FALSKY_COMMENTS_PER_PAGE'],
        error_out=False)
    comments = pagination.items
